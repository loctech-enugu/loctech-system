...
import { useGetReports } from "@/hooks/use-staff-attendance";
import { ReportDetails } from "./ReportDetails";

export const CalendarOfStaffAttendance = () => {
  ...
  const getReportsForDate = (date: Date) => {
    ...
    return (
      data?.data?.filter(
        (report) => format(new Date(report.time), "yyyy-MM-dd") === dateStr
      ) || []
    );
  };
                    ...
                    {dayReports.slice(0, 2).map((attendance) => (
                      <div
                        ...
                        className={cn(
                          ...
                          attendance.isExcused && "bg-primary-500",
