...
import { useGetReports } from "@/hooks/use-student-attendance";
import { AttendanceDetails } from "./day-details";

export const CalendarOfStudentAttendance = ({ courseId = "" }) => {
  ...
  const { data, isPending } = useGetReports(courseId, startDate, endDate);
  ...
  const getReportsForDate = (date: Date) => {
    ...
    return (
      data?.data?.filter(
        (report) => format(new Date(report.createdAt), "yyyy-MM-dd") === dateStr
      ) || []
    );
  };
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-1">
                    {dayReports.slice(0, 2).map((attendance) => (
                      <div
                        key={attendance.id}
                        className={cn(
