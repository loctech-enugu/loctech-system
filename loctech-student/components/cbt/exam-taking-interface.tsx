...
async function getExamStatus(examId: string, userExamId: string) {
  const res = await fetch(`/api/student/exams/${examId}?userExamId=${userExamId}`);
  if (!res.ok) {
    const data = await res.json();
    throw new Error(data.error || "Failed to get exam status");
  }
  return res.json();
}
...
export default function ExamTakingInterface({
  ...
  // Get exam status if userExamId exists
  const { data: examStatus } = useQuery({
    queryKey: ["exam-status", examId, userExamId],
    queryFn: () => getExamStatus(examId, userExamId!),
    enabled: !!userExamId && examStarted,
    refetchInterval: 30000, // Refetch every 30 seconds
  });

  const userExam = examData?.data || examStatus?.data;
  const questions = userExam?.questions || [];
  ...
  const {
    ...
    onViolation: (violation) => {
      if (userExamId) {
        recordViolation(userExamId, violation);
      }
      setShowWarning(true);
    },
    onMaxViolations: () => {
  ...
  // Handle time expiry
  useEffect(() => {
    if (!userExam?.startTime || !userExam?.exam?.duration) return;
    
    const startTime = new Date(userExam.startTime).getTime();
    const durationMs = userExam.exam.duration * 60 * 1000;
    const endTime = startTime + durationMs;
    const now = Date.now();

    if (now >= endTime && !hasAutoSubmitted && examStarted) {
      handleAutoSubmit("time_up");
    }
  }, [userExam, hasAutoSubmitted, examStarted]);
  ...
  const handleAnswerChange = (value: string | string[]) => {
    ...
    // Auto-save answer
    if (userExamId) {
      saveAnswer(userExamId, currentQuestion.id, value).catch((error) => {
  ...
  const handleAutoSubmit = async (
    reason: "manual" | "time_up" | "tab_switch_violation" = "manual"
  ) => {
    if (!userExamId || hasAutoSubmitted) return;
    ...
    try {
      ...
      if (reason !== "manual") {
        toast.error(
          `Exam automatically submitted due to ${reason === "time_up" ? "time expiry" : "security violations"}`
        );
      }
      router.push(`/dashboard/exams/${examId}/results`);
    } catch (error: any) {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <ExamTimer
                ...
                onTimeUp={() => handleAutoSubmit("time_up")}
              />
              {violationCount > 0 && (
